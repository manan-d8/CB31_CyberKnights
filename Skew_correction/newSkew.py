...
class newSkew:
    ...
    def StartProcess(self, Plate):
        ...
        finalRes=[]
        print("2")
        if(len(self.ResArr)>0):
            iterArr=deepcopy(self.ResArr)
            sorted(iterArr,key=getArea,reverse=True)
            print("3")
            if(self.hasRT(iterArr[0])):
                self.imgRS=iterArr[0]
                anglex, self.imgRT = self.Rotate()
                newRes.append(self.imgRT)
            if(len(iterArr)>1 and len(newRes)==0):
                if(self.hasRT(iterArr[1])):
                    self.imgRS=iterArr[1]
                    anglex, self.imgRT = self.Rotate()
                    newRes.append(self.imgRT)
            if(len(newRes)==0):
                newRes.append(self.origRT)
            print("4")
        else:
            newRes.append(self.origRT)
        # op=[]
        ...
        #newRes[0]=newRes[0]*0.5
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        # R, G, B = cv2.split(newRes[0])
