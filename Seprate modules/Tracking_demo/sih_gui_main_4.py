...
print(Base_Dir)
visiterFolder = '/home/manand8/dev/sih/SIH_APP_CLIENT/client-caffe/VisitorStored/'
...
class TopFrame(tk.Frame):
	def __init__(self,parent,*args,**kwargs):
		...
		mylbl(self, text= Heading,bg=clr1, fg=clr3, font=('Arial', 30,"bold"),relief=tk.RAISED,bd=3).pack(fill=tk.X)

class MidFrame(tk.Frame):
	def __init__(self,parent,*args,**kwargs):
		tk.Frame.__init__(self,parent,*args,**kwargs)
		self.parent = parent
		MidLeftFrame(self,bg=clr2,relief=tk.RIDGE,bd=1).pack(side = tk.TOP,fill = tk.BOTH)
...
class Detector():
	def __init__(self):
		...
		self.CLASSES = ["background", "car","bicycle","motorbike","bus"]
