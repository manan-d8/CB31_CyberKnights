...
class Char_Detector:
	def __init__(self):
		...
		self.modelConfiguration = r"Cfg\YoloV3SIH(chars).cfg"
		self.modelWeights = r"Weights\YoloV3SIH(chars)_19000.weights"
...
if __name__ == '__main__':
	no = 0
	ChdObj = Char_Detector()
	img_lis = glob.glob(r'Test_Images\\*.jpg')
	for img_path in img_lis:
		no += 1
		frame = cv2.imread(img_path)
		ret = ChdObj.Check_if_Char_Exist(frame)
		H,W,_ = frame.shape
		if ret[0]:
			chars = ret[1]
			for char in chars:
				print(' [ Acc  ] ' , str( classes[char[4]]) , ' : ' ,str(char[5]))
		...
		cv2.imwrite('Results/CharDetect'+str(no)+'.jpg',frame)
